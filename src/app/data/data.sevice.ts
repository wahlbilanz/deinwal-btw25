...
export class DataService {
    ...
    private readonly kategorien: string[] = Array.from(new Set(this.fragen.map(frage => frage.kategorie))).sort();
    private readonly fraktionen: string[] = Array.from(new Set(Object.values(this.ergebnisse).map(ergebnis => Object.values(ergebnis).map(e => e.fraktion)).flat())).filter(fraktion => fraktion !== "Fraktionslos").sort();
